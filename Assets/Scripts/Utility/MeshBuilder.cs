...
public class MeshBuilder
{
    ...
    public void AddVerts(Vector3[] verts, int[] triangles, Vector2[] uvs)
    {
        var offset = this.verts.Count;
        for (int i = 0; i < verts.Length; i++)
        {
            this.verts.Add(verts[i]);
            this.vertsColor.Add(Color.white);
            this.uvs.Add(uvs[i]);
        }
        for (int i = 0; i < triangles.Length; i++)
            this.trianglesIndex.Add(offset + triangles[i]);
    }
